...
set.seed(123)
index <- createDataPartition(film$Rating, p = 0.8, list = FALSE)
film_train <- data[index, ]
film_test <- data[-index, ]
...
```{r}
pred_prob_model1_1 <- predict(model1_1, newdata = test_data, type = "response")

```

```{r}
# Compute precision for each model
recall_model1_1 <- conf_matrix_model1$byClass["Recall"]
recall_model1_2 <- conf_matrix_model2$byClass["Recall"]
recall_model2 <- conf_matrix_model3$byClass["Recall"]

# Compute accuracy for each model
accuracy_model1_1 <- conf_matrix_model1$overall["Accuracy"]
accuracy_model1_2 <- conf_matrix_model2$overall["Accuracy"]
accuracy_model2 <- conf_matrix_model3$overall["Accuracy"]
